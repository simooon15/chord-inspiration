...
stringRange → 控制该 slot 扫哪几根弦
  ...
  ├─ [0,2]=只取低音区音符（大鼓感，收着弹）
  └─ [3,5]=只取高音区音符（清脆，副歌炸开）
  实现方式：将排序后的 MIDI 音符低→高索引切片（不依赖音高），保证所有和弦 voicing 一致
...
- **弦索引**: 0=最低音，5=最高音（对应排序后 MIDI 音符数组下标，不依赖具体音高）
- **实现**: 将排序后的 MIDI 音符低→高索引切片（`sortedAsc.slice(lowIdx, highIdx + 1)`），`lowIdx = round(lowStr/5 × (n-1))`，`highIdx = round(highStr/5 × (n-1))`
- **不填**: 默认为 `[0, 5]`（全扫）
- **听感**: 同一和弦在不同拍子上有厚/薄层次对比，所有和弦 voicing 一致
